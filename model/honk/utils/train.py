...
def evaluate(config, model=None, test_loader=None):
    ...
    def count_parameters(model):
        ...
        return mem
    print("Model size (bytes):",count_parameters(model))
    import pytorch2timeloop
    pytorch2timeloop.convert_model(model, (101, 40), 1, 'workloads', 'honk', True, [])
    exit # All we need is the model size and timeloop files
